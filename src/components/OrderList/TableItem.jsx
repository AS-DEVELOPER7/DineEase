...
function TableItem({data,id}) {
  ...
  return (
    <div className={`${id%2===0?'bg-slate-100':''} flex rounded-xl items-center justify-between p-2`}  >
            ...
            <div className="w-1/6 flex justify-center text-blue cursor-pointer" onClick={()=>handleModal()} ><div className={`${id%2===0?'bg-white':'bg-slate-100'} p-1 rounded-md`}><AiFillEye size={20}/></div></div>
          </div>
  )
}
